#include <iostream>
#include <string>

using namespace std;

// Superclass
class Investment{
public:
    
    Investment(string itemName, int itemPrice) : invName(itemName), price(itemPrice) {}
    virtual ~Investment(){}
    
    string name() const {return invName;}
    int purchasePrice() const {return price;}
    virtual bool fungible() const = 0;
    virtual string description() const = 0;
    
private:
    
    string invName;
    int price;
};

// Sublclass of Investment
class Painting : public Investment {
public:
    
    Painting(string itemName, int itemPrice) : Investment(itemName, itemPrice){}
    virtual ~Painting(){cout << "Destroying " + name() + ", a " + description() << endl;}
    
    virtual bool fungible() const {return false;}
    virtual string description() const {return "painting";}
    
private:
};

// Sublclass of Investment
class Stock : public Investment {
public:
    
    Stock(string itemName, int itemPrice, string stockSymbol) : Investment(itemName, itemPrice),
        symbol(stockSymbol) {}
    virtual ~Stock(){cout << "Destroying " + name() + ", a stock holding" << endl;}
    
    virtual bool fungible() const {return true;}
    virtual string description() const {return "stock trading as " + symbol;}
    
private:
    
    string symbol;
};

// Sublclass of Investment
class House : public Investment {
public:
    
    House(string itemName, int itemPrice) : Investment(itemName, itemPrice) {}
    virtual ~House(){cout << "Destroying the house " + name() << endl;}
    
    virtual bool fungible() const {return false;}
    virtual string description() const {return "house";}
    
private:
};

// Display Investement attributes
void display(const Investment* inv)
{
    cout << inv->name();
    if (inv->fungible())
        cout << " (fungible)";
    cout << ": " << inv->description() << " bought for $" << inv->purchasePrice() << endl;
}

int main()
{
    Investment* portfolio[4];
    portfolio[0] = new Painting("Salvator Mundi", 450300000);
    // Stock holdings have a name, value, and ticker symbol
    portfolio[1] = new Stock("Alphabet", 100000, "GOOGL");
    portfolio[2] = new Stock("Symantec", 50000, "SYMC");
    portfolio[3] = new House("4 Privet Drive", 660000);
    
    for (int k = 0; k < 4; k++)
        display(portfolio[k]);
    
    // Clean up the investments before exiting
    cout << "Cleaning up" << endl;
    for (int k = 0; k < 4; k++)
        delete portfolio[k];
}